...
impl TradingEngine {
    ...
    pub async fn execute_buy(&self, order: &mut ghostRbot_core::Order, wallet_key: &str) -> Result<(), AppError> {
        ...
        order.status = ghostRbot_core::OrderStatus::Monitoring;
    ...
    pub async fn execute_sell(&self, order: &mut ghostRbot_core::Order, wallet_key: &str) -> Result<(), AppError> {
        let client = self.client_for_chain(&order.chain);
        ...
        order.status = ghostRbot_core::OrderStatus::Sold;
        order.updated_at = chrono::Utc::now();
    ...
    pub async fn monitor_order(&self, order: &mut ghostRbot_core::Order) -> Result<(), AppError> {
        if order.status == ghostRbot_core::OrderStatus::Error
            || order.status == ghostRbot_core::OrderStatus::Sold
            || order.status == ghostRbot_core::OrderStatus::Cancelled
        {
            return Ok(());
        }
